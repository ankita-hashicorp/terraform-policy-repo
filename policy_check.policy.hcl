...
resource_policy "random_id" "byte_length_check" {
  ...
  enforce {
    ...
    info_message = "byte_length must be 8. Current value: ${attrs.byte_length} and input ${input.param1}. stack name is ${meta.stack_name}"
  }
}
...
resource_policy "random_pet" "length_prefix_check" {
  ...
  enforce {
    ...
    info_message = "length must be 3 and prefix must be 'dev'. Current values: length=${attrs.length}, prefix=${attrs.prefix}. stack deployment name is ${meta.deployment_name}"
    error_message = "length must be 3 and prefix must be 'dev'. Current values: length=${attrs.length}, prefix=${attrs.prefix}. stack deployment name is ${meta.deployment_name}"
  }
}
...
resource_policy "random_id" "random_id_check" {
  ...
  enforce {
    condition     = attrs.id != ""
    info_message = "id must be present. stack deployment name is ${meta.stack_name}"
