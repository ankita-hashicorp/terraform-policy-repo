...
resource_policy "aws_s3_bucket" "tag_owner_check" {
  ...
  enforce {
    condition     = core::try(attrs.tags.Owner != "", false) && core::try(meta.tags.test_tag == "test_value", false)
    error_message = "bucket must have an owner tag. Current value: ${attrs.tags.Owner} && test_tag value is ${meta.tags.test_tag}"
    info_message = "Bucket must have an owner tag. Current owner value: ${attrs.tags.Owner} && test_tag value is ${meta.tags.test_tag}"
  }
}
